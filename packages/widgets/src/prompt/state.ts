...
import type { WidgetPromptTarget } from "./host";
...
export function deriveActiveToolLine(entries: AgentEntry[]): string | null {
  for (let i = entries.length - 1; i >= 0; i--) {
    ...
    const file = path ? basename(path).trim() : "";
 ...
 *
 * `toRelativePath` is passed in rather than imported: path spelling is a
 * platform fact (posix vs win32) the host owns, and taking it as an argument
 * keeps this function testable without one.
 */
